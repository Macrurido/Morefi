...
# Place this chunk at the top of your R Markdown file.
## Set the default echo to FALSE 

# knitr::opts_chunk$set() is used to set global options for all chunks in the document.

# If you need to override this setting for specific chunks, you can explicitly set
#echo = TRUE in those chunks.
...
This is a methodological package developed in R for analyzing the article:

Aguirre-Villaseñor, H., Morales-Bojórquez, E., & Cisneros-Mata, M. Á. (2025). Biometric relationships as a fisheries management tool: A case study on the bullseye puffer (*Sphoeroides annulatus*) in an artisanal fishery. Fisheries Research, 292, 107593. [DOI](https://doi.org/10.1016/j.fishres.2025.107593)
...
The package contains two data sets. Data are available under the terms of the [The Creative Commons Attribution 4.0 International (CC BY 4.0)](https://creativecommons.org/licenses/by/4.0/legalcode).

### Bullseye puffer measures

To demonstrate how the package functions, we utilize the dataset `botete`, containing 1,397 fish across 7 variables: the total length(LT), standard length (SL), body trunk length (LB), total weight (WT), body trunk weight (WB), fillet weight (Wfi) and Fleet of the bullseye puffer (*Sphoeroides annulatus*), collected from the Eastern Central Pacific. In this dataset, the landing category is included in the "Fleet" variable, which is categorized as follows: 1 indicates Fresh, while 2 denotes Frozen-thawed. This biometric data file, is included in the package with permission from the Instituto Mexicano de Investigación en Pesca y Acuacultura Sustentables (Mexican Institute for Research in Sustainable Fishing and Aquaculture). 
...
# Values to define for the examples{#Values_to_define}

[Esto no lo se Rick](#No_lo_se_Rick)

The following objects, data frames, and lists will be utilized in multiple examples to avoid repetition of information.
...
```{r fn_ARSS, warning=FALSE, message=FALSE}
...
```
> **&#9432;** To enhance the visualization of the function results, tables and their headers were constructed using the knitr and kableExtra packages. The code is not displayed.
...
```{r fn_dfa, message=FALSE, warning=FALSE}
library(dplyr)
library(tibble)
...
```{r fn_freqw, message=FALSE, warning=FALSE}
 ...
 * `modelos` A list defining the numerical column variables for the model.
 * `bin` A vector contains the step size for each independent variable in every model.
...
Using biometric data from `Morefi::botete`, the sequence of independent variables for the relationships between Total Length and Standard Length (LT_LS) and between Total Length and Total Weight (LT_WT) was computed using the function `fn_xseq`.

The object `modelos` was defined earlier in the section titled **Values to Define for the Examples**.

```{r fn_xseq, message=FALSE, warning=FALSE}
df <- mydata
bin <- c(2)
i=1
...
```{r , echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
xseq <- t(cbind(head(xseq1, 9), head(xseq2, 9)))

kable(xseq[,1:9], format = "html", caption = "The first nine columns of the table are displayed, based on the sequence of independent variables for the relationships between Total Length and Standard Length (LT_LS), and between Total Length and Total Weight (LT_WT).")
```
> **&#9432;** To enhance the visualization of the function results, tables and their headers were constructed using the knitr and kableExtra packages. The code is not displayed.
...
```{r fn_intervals, message=FALSE, warning=FALSE}
...
```{r , echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
kable(CI[1:6,], format = "html", caption = "The first six rows of the table display the results of the LT versus LS relationship. In this context, x1 represents the independent variable, while fit refers to the fitted values. Additionally, CI_L indicates the lower confidence interval, and CI_U indicates the upper confidence interval. Similarly, PI_L represents the lower predicted interval, and PI_U denotes the upper predicted interval.")
```
> **&#9432;** To enhance the visualization of the function results, tables and their headers were constructed using the knitr and kableExtra packages. The code is not displayed.
...
Fillet yield was calculated for a sequence of the independent variable (including minimum and maximum values). The function `fn_fyield()` returms a table with fillet yield results.
...
`CI` comes from `fn_intervals()` example

```{r fn_fyield, message=FALSE, warning=FALSE}
bm <- 18
df <- CI
i <- 1
...
kable(yield1[1:10,], format = "html", caption = "The first six rows of the table are displayed, based on the LT versus LS relationship results.  In this context, x1 represents the independent variable, while fit refers to the fitted values. Additionally, CI_L indicates the lower confidence interval, and CI_U indicates the upper confidence interval. Similarly, PI_L represents the lower predicted interval, and PI_U denotes the upper predicted interval.") 

kable(yield2[1:10,], format = "html", caption = "The first six rows of the table are displayed, based on the LT versus WT relationship results.  In this context, x1 represents the independent variable, while fit refers to the fitted values. Additionally, CI_L indicates the lower confidence interval, and CI_U indicates the upper confidence interval. Similarly, PI_L represents the lower predicted interval, and PI_U denotes the upper predicted interval.")
```
> **&#9432;** To enhance the visualization of the function results, tables and their headers were constructed using the knitr and kableExtra packages. The code is not displayed.
...
The function returns a vector containing both $R^{2}$ and the adjusted $R^{2}_{adj}$ values.
 
 
```{r fn_R2RV, message=FALSE, warning=FALSE}
R2wa <- fn_R2RV(eq=eq2)
```

### fn_summary()

The function creates a customized table that displays the regression summary categorized by model and landed weight types: Fresh, Frozen-thawed (Frozen), and Total. The 95% confidence interval is noted in parentheses. Additionally, the adjusted coefficient of determination (R2) and the degrees of freedom (DF) are also reported.
...
# Aqui
<!-- La funcion esta definida para botete, si tengo diferentes modelos no jala, utilizo  -->
<!-- ifelse (i<7,"Linear","Potential")
EN el ejemplo de este manual solo uso dos una lineal y otra potencial, sin embargo en LT vs WT dice lineal porque i = 2.
Si quiero ver la tabla no la veo directo porque me da la Lista
si quiero ver la tabla la tengo que hacer lo que hago en Filete.

Tengo que ver si en la función la dejo en la lista la tabla o solo
hago la tabla y la guardo en la lista por fuera como en el resto de los ejemplos
-->
...
```{r fn_summary, message=FALSE, warning=FALSE}
...
# Caption
L_T_sum <- paste("Regression summarized by model and landed
                    weight categories: Fresh, Frozen-thawed
                    (Frozen) and Total, for the bullseye puffer
                    <i>Sphoeroides annulatus</i>. The 95%
                    confidence interval parameter is in
                    parentheses. Adjusted robust coefficient of
                    determination (", paste("$R^{2}_{adj}$"),
                    paste(") and Degrees of Freedom (DF) are also
                    reported. Acronyms used: Total length "),
                    paste("($L_{T}$)"),
                    paste("standard length "), paste("($L_{S}$)"),
                    paste("body trunk length "),paste("($L_{B}$)"),
                    paste("total weight "),paste("($W_{T}$)"),
                    paste("body trunk weight "),paste("($W_{B}$)"),
                    paste("and fillet weight "), 
                    paste("($W_{fi}$)."))  
  


kable(List_Tables[[1]], format = "html", caption = L_T_sum) 
...
`catch` cames from Data available in the package/data Bullseye puffer fish landings explanation.

```{r fn_Wlive, message=FALSE, warning=FALSE}
...
```{r df_plots, message=FALSE, warning=FALSE}
...
```{r order, message=FALSE, warning=FALSE}
...
```{r my_labeller, message=FALSE, warning=FALSE}
...
The x-axis label is defined in the lab_x object, while the y-axis label is defined in the lab_y object. In this example, both labels are italicized, with the i-th label formatted as a subscript.
...
Aguirre-Villaseñor, H., Morales-Bojórquez, E., & Cisneros-Mata, M. Á. (2025). Biometric relationships as a fisheries management tool: A case study on the bullseye puffer (*Sphoeroides annulatus*) in an artisanal fishery. Fisheries Research, 292, 107593.
...
Aguirre-Villaseñor H (2025). Morefi: Morphological Relationships Fitted by Robust Regression. R package version 0.1.0, https://macrurido.github.io/Morefi/.
...
<!-- A ver si si [Values to define for the examples](#Values_to_define) -->
...
<!-- ```{r Nell_T1, warning=FALSE, message=FALSE} -->
<!--     # An empty table to store the summary regression results    -->

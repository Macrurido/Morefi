...
```{r , echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
xseq <- cbind(head(xseq1, 6), head(xseq2, 6))

kable(xseq[1:6,], format = "html", caption = "The first six rows of the table are displayed, based on the sequence of independent variables for the relationships between Total Length and Standard Length (LT_LS), and between Total Length and Total Weight (LT_WT).")
```
...
```{r fn_fyield, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
bm <- 18
df <- CI
i <- 1
...
# Aqui
<!-- La funcion esta definida para botete, si tengo diferentes nodelos no jala, utilizo  -->
<!-- ifelse (i<7,"Linear","Potential") -->

```{r fn_summary, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

#List_Tables <- list(eq1, eq2)
#modelos <- modelos
#catego <- c("Total")
#eq <-
#R2 <- 
...
```{r fn_Wlive, , echo=FALSE, message=FALSE, warning=FALSE, results='asis' }
